...
try:
    #배경화면
    background_img = pygame.image.load("hangman_background.png").convert()
    ...
    head_img = pygame.transform.scale(head_img, (500, 600))  # 크기 조정

    left_arm_img = pygame.image.load("hangman_right_arm.png").convert_alpha()
    left_arm_img = pygame.transform.scale(left_arm_img, (500, 600))

    body_img = pygame.image.load("hangman_body.png").convert_alpha()
    body_img = pygame.transform.scale(body_img, (500, 600))

    right_arm_img = pygame.image.load("hangman_left_arm.png").convert_alpha()
    right_arm_img = pygame.transform.scale(right_arm_img, (500,600))

    left_leg_img = pygame.image.load("hangman_left_leg.png").convert_alpha()
    left_leg_img = pygame.transform.scale(left_leg_img, (500, 600))

    right_leg_img = pygame.image.load("hangman_right_leg.png").convert_alpha()
    right_leg_img = pygame.transform.scale(right_leg_img, (500, 600))
...
while True:
    ...
    # 고정된 위치에 행맨 그림을 그리기
    hangman_x = 0 #left_width // 8  # 그림을 왼쪽 1/4 지점에 배치
    hangman_y = 0  # 그림의 y 좌표 고정
    ...
    word_text = font.render(word_display, True, SIAN)
    ...
    #사용자가 입력한 글자 화면 표
    user_input_text = small_font.render(f"Your Input: {user_input}", True, BROWN)
    screen.blit(user_input_text, (left_width + 50, 250))


    # 힌트 버튼 텍스트
    hint_text = small_font.render("Press H for Hint", True, BEIGE)
    ...
    if lives == 0:
        ...
        restart_text = font.render("Press R to Restart", True, BEIGE)
        ...
        restart_text = font.render("Press R to Restart", True, BEIGE)
        screen.blit(restart_text, (SCREEN_WIDTH // 1 - restart_text.get_width() // 2, 400))
